...
| **Reddit** | Searches Reddit for the top clusters — checks for pain signals ("wish there was", "looking for", "frustrated") |
| **Time series** | Fetches 30-day trend data for top ~15 keywords to refine freshness scores |
| **Briefing** | Writes `briefings/briefing_YYYY-MM-DD.md` — cluster table, the story, build opportunities |

Total API calls per run: ~7 (no paid APIs, no auth required except Gmail).
